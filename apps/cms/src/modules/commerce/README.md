...
(WhatsApp), D8 (the inbox), D9 (consent-gated campaigns), and D2/D3's
session half (payment-gateway session creation + webhook-endpoint tokens,
Midtrans Snap) are IMPLEMENTED, not contract-only; see their own sections
below.** Every one of
...
gateway webhook intake + reconciliation (#113) — the set is required to be
...
campaigns', and the payment-gateway session half's own exemption entries
are already removed (#107, #108, #111, #114, #110). ADR-0017 names the
webhook INTAKE route's own environment concerns (the
