...
## External providers — contract only, except D4, D5, D8, and D2/D3's session half (epic #33 wave 0 — ADR-0017, issue #106)
...
`reporting`-hosted sales projections (D7), consent-gated campaigns (D9),
and the module-settings feature flags plus tiered pricing at quote (D10).
**D4 (courier rates), D5 (WhatsApp), D8 (the inbox), and D2/D3's session
half (payment-gateway session creation + webhook-endpoint tokens, Midtrans
Snap) are IMPLEMENTED, not contract-only; see their own sections below.**
Every one of
...
the child issue that removes it: POS (#116), sales reports (#117), and
...
discipline #86/ADR-0016 already proved for accounts. Courier rates',
WhatsApp's, the inbox's, and the payment-gateway session half's own
exemption entries are already removed (#107, #108, #111, #110).
