...
campaigns), and D2/D3's session half (payment-gateway session creation +
webhook-endpoint tokens, Midtrans Snap) are IMPLEMENTED, not
...
the child issue that removes it: POS (#116) and gateway webhook intake +
...
accounts. Courier rates', WhatsApp's, the inbox's, campaigns', the
payment-gateway session half's, and the sales reports' own exemption
entries are already removed (#107, #108, #111, #114, #110, #117). ADR-0017 names the
