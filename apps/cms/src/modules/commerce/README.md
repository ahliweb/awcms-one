...
## External providers — contract only, except D4, D5, D8, D9, and D2/D3's session half (epic #33 wave 0 — ADR-0017, issue #106)
...
(WhatsApp), D8 (the inbox), D9 (consent-gated campaigns), and the FULL
payment gateway (D2/D3 — session creation, webhook-endpoint tokens, webhook
intake, reconcile) are IMPLEMENTED, not contract-only; see their own
sections below.** Every one of
...
the child issue that removes it: POS (#116) and sales reports (#117) —
...
WhatsApp's, the inbox's, campaigns', and the full payment gateway's
(session half AND webhook intake/reconcile) own exemption entries are
already removed (#107, #108, #111, #114, #110, #113). The RajaOngkir env vars
