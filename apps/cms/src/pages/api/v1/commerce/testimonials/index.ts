...
export const GET = defineTenantRoute({
  ...
  handler: async ({ tx, tenantId, prepared }) =>
    ok(await listTestimonials(tx, tenantId, prepared.cursor))
});

export const POST = defineTenantRoute({
  workClass: "interactive",
  prepare: async ({ request }): Promise<CreateTestimonialInput | Response> => {
    const bodyRead = await readJsonBody(request);
    if (bodyRead.tooLarge) return bodyTooLargeResponse(bodyRead.limitBytes);

    const validation = validateCreateTestimonialInput(bodyRead.value);
    if (!validation.valid) {
      return fail(
        400,
        "VALIDATION_ERROR",
        "Testimonial creation input is invalid.",
        {},
        validation.errors
      );
    }
