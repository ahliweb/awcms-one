...
const IDEMPOTENCY_SCOPE = "commerce_affiliate_commission_pay";
...
/** `POST /api/v1/commerce/affiliate-commissions/{id}/pay` — `approved -> paid` (Issue #92). High-risk mutation: requires `Idempotency-Key`. */
export const POST = defineTenantRoute<Prepared>({
  ...
  handler: async ({ tx, tenantId, auth, params, prepared, locals }) => {
    ...
    const requestHash = computeRequestHash({ commissionId, action: "pay" });
    ...
    try {
      const updated = await payAffiliateCommission(
        tx,
        tenantId,
      ...
      if (error instanceof InvalidCommissionTransitionError) {
        return fail(409, "COMMISSION_NOT_PAYABLE", error.message);
      }
      throw error;
    }
  }
