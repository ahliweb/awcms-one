...
const IDEMPOTENCY_SCOPE = "commerce_affiliate_commission_approve";
...
/** `POST /api/v1/commerce/affiliate-commissions/{id}/approve` — `pending -> approved` (Issue #92). High-risk mutation: requires `Idempotency-Key`. */
export const POST = defineTenantRoute<Prepared>({
  ...
  handler: async ({ tx, tenantId, auth, params, prepared, locals }) => {
    ...
    try {
      const updated = await approveAffiliateCommission(
        tx,
        tenantId,
      ...
      if (error instanceof InvalidCommissionTransitionError) {
        return fail(409, "COMMISSION_NOT_APPROVABLE", error.message);
      }
      throw error;
    }
  }
