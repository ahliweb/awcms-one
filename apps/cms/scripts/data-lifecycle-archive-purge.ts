 ...
 * (`src/modules/data-lifecycle/application/archive-purge-job.ts`) — same shape as
 ...
 * descriptor (both `"delegated"` and `"generic"`) without any mutation — safe to
...
async function main() {
  // `awcms_worker` role (sql/022) — see migration 055's own grants.
