...
Yang dikirim skrip itu hari ini ([ADR-0123](../adr/0123-backup-encryption-manifest-authentication.id.md)): secara default, dump `--format=custom` polos plus sidecar `.sha256`, dan pemangkasan retensi (`BACKUP_RETENTION_DAYS`, default 14) — semuanya berjalan penuh **tanpa internet**, jadi profil offline/LAN tidak kehilangan apa pun dan tanpa secret yang perlu dikelola. Ketika host punya cerita secret-management, men-set `BACKUP_AGE_RECIPIENTS_FILE` + `BACKUP_HMAC_KEY_FILE` bersama menyalakan enkripsi-at-rest `age` plus manifest pemulihan terautentikasi HMAC-SHA256 (`deploy/backup/manifest.sh`); men-set hanya salah satunya gagal tertutup alih-alih diam-diam jatuh ke plaintext. `deploy/backup/offsite-copy.sh` adalah adapter rsync/SSH nyata dengan retry yang tidak pernah menghapus salinan lokal apa pun hasil transfernya, dan `deploy/backup/restore-drill.sh` adalah restore drill tanpa-pengawasan nyata, siap cron/CI (memilih backup eligible terbaru, memverifikasi + mendekripsi, merestore ke database scratch, mencatat bukti JSON-lines) tanpa jalur kode yang bisa menyasar produksi — lihat `deploy/backup/README.md` untuk panduan operator lengkap.

Juga terkirim: `bun run resilience:dr-drill` tetap orkestrator yang belum dibangun (lihat `resilience-dr-verification.md`, yang bannernya sendiri masih menyatakan begitu) yang diharapkan memanggil `restore-drill.sh` begitu ia ada — asurans backup/restore repo ini tidak menunggu framework failure-injection yang lebih besar itu.
