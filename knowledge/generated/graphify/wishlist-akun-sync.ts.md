---
source_file: "apps/storefront/src/lib/wishlist-akun-sync.ts"
type: "code"
community: "wishlist-akun-sync.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/wishlist-akun-syncts
---

# wishlist-akun-sync.ts

## Connections
- [[AKUN_EVENT_NAME]] - `imports` [EXTRACTED]
- [[akun-klien.ts]] - `imports_from` [EXTRACTED]
- [[akun-kontrak.ts]] - `imports_from` [EXTRACTED]
- [[akun-sesi.ts]] - `imports_from` [EXTRACTED]
- [[bacaSesi()]] - `imports` [EXTRACTED]
- [[gabungkanWishlist()]] - `imports` [EXTRACTED]
- [[hapusWishlistAkunItem()]] - `imports` [EXTRACTED]
- [[laporkanKegagalan()]] - `contains` [EXTRACTED]
- [[loadWishlist()]] - `imports` [EXTRACTED]
- [[pasangSinkronisasiWishlist()]] - `contains` [EXTRACTED]
- [[saveWishlist()]] - `imports` [EXTRACTED]
- [[simpanWishlistAkun()]] - `imports` [EXTRACTED]
- [[sinkronkanWishlistSaatMasuk()]] - `contains` [EXTRACTED]
- [[statusElement()]] - `contains` [EXTRACTED]
- [[tulisKeAkunJikaMasuk()]] - `contains` [EXTRACTED]
- [[wishlist-klien.ts]] - `imports_from` [EXTRACTED]
- [[wishlist-sinkron.ts]] - `imports_from` [EXTRACTED]
- [[wishlist-tombol.ts]] - `imports_from` [EXTRACTED]
- [[wishlist.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/wishlist-akun-syncts