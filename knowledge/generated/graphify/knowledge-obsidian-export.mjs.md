---
source_file: "tools/knowledge-obsidian-export.mjs"
type: "code"
community: "Obsidian Export Safety"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Obsidian_Export_Safety
---

# knowledge-obsidian-export.mjs

## Connections
- [[ALLOWED_TO_CLEAR]] - `contains` [EXTRACTED]
- [[CURATED]] - `contains` [EXTRACTED]
- [[GENERATED]] - `contains` [EXTRACTED]
- [[STAGING]] - `contains` [EXTRACTED]
- [[checkCuratedCollision()]] - `imports` [EXTRACTED]
- [[classifyEntry()]] - `imports` [EXTRACTED]
- [[entries_1]] - `contains` [EXTRACTED]
- [[fail()_1]] - `contains` [EXTRACTED]
- [[obsidian-safety.mjs]] - `imports_from` [EXTRACTED]
- [[proc_1]] - `contains` [EXTRACTED]
- [[rejections]] - `contains` [EXTRACTED]
- [[resolveWithin()]] - `imports` [EXTRACTED]
- [[syncable]] - `contains` [EXTRACTED]
- [[walk()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Obsidian_Export_Safety