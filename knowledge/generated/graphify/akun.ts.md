---
source_file: "apps/storefront/src/scripts/akun.ts"
type: "code"
community: "Account Dashboard Script"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Account_Dashboard_Script
---

# akun.ts

## Connections
- [[AKUN_EVENT_NAME]] - `imports` [EXTRACTED]
- [[LEVEL_LABELS]] - `contains` [EXTRACTED]
- [[TokoApiError]] - `imports` [EXTRACTED]
- [[akun-klien.ts]] - `imports_from` [EXTRACTED]
- [[akun-kontrak.ts]] - `imports_from` [EXTRACTED]
- [[akun-sesi.ts]] - `imports_from` [EXTRACTED]
- [[akunindex.astro]] - `imports_from` [EXTRACTED]
- [[ambilProfil()]] - `imports` [EXTRACTED]
- [[bacaSesi()]] - `imports` [EXTRACTED]
- [[buildWhatsappAccountMessage()]] - `imports` [EXTRACTED]
- [[buildWhatsappUrl()]] - `imports` [EXTRACTED]
- [[hapusSesi()]] - `imports` [EXTRACTED]
- [[hideSubmitError()_4]] - `contains` [EXTRACTED]
- [[keluar()]] - `imports` [EXTRACTED]
- [[levelLabel()]] - `contains` [EXTRACTED]
- [[render()_6]] - `indirect_call` [INFERRED]
- [[renderProfile()]] - `contains` [EXTRACTED]
- [[root_9]] - `contains` [EXTRACTED]
- [[showAccountView()_2]] - `contains` [EXTRACTED]
- [[showGuestView()_3]] - `contains` [EXTRACTED]
- [[showSubmitError()_4]] - `contains` [EXTRACTED]
- [[showWaFallback()]] - `contains` [EXTRACTED]
- [[toko-permintaan.ts]] - `imports_from` [EXTRACTED]
- [[ubahProfil()]] - `imports` [EXTRACTED]
- [[wa-fallback.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Account_Dashboard_Script