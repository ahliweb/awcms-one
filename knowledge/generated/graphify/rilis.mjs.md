---
source_file: "tools/rilis.mjs"
type: "code"
community: "Release & Changeset Versioning"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Release__Changeset_Versioning
---

# rilis.mjs

## Connections
- [[BUMP_LEVELS]] - `imports` [EXTRACTED]
- [[apply]] - `contains` [EXTRACTED]
- [[args]] - `contains` [EXTRACTED]
- [[atLeastAsSignificant()]] - `imports` [EXTRACTED]
- [[body]] - `contains` [EXTRACTED]
- [[bumpVersion()]] - `imports` [EXTRACTED]
- [[changeset.mjs]] - `imports_from` [EXTRACTED]
- [[changesetBody()]] - `imports` [EXTRACTED]
- [[changesetContent]] - `contains` [EXTRACTED]
- [[commit]] - `contains` [EXTRACTED]
- [[derivedLevel]] - `contains` [EXTRACTED]
- [[formatTag()]] - `imports` [EXTRACTED]
- [[found]] - `contains` [EXTRACTED]
- [[git()]] - `contains` [EXTRACTED]
- [[git.mjs]] - `imports_from` [EXTRACTED]
- [[gitRunInherit()]] - `imports` [EXTRACTED]
- [[gitRunOrThrow()]] - `imports` [EXTRACTED]
- [[highestBump()]] - `imports` [EXTRACTED]
- [[invalid]] - `contains` [EXTRACTED]
- [[isChangesetFile()]] - `indirect_call` [INFERRED]
- [[next]] - `contains` [EXTRACTED]
- [[parseChangeset()]] - `imports` [EXTRACTED]
- [[pkg_1]] - `contains` [EXTRACTED]
- [[requestedBumps]] - `contains` [EXTRACTED]
- [[requestedLevel]] - `contains` [EXTRACTED]
- [[semver.mjs]] - `imports_from` [EXTRACTED]
- [[tag]] - `contains` [EXTRACTED]
- [[today]] - `contains` [EXTRACTED]
- [[validateChangeset()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Release__Changeset_Versioning