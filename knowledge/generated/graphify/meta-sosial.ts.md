---
source_file: "apps/storefront/src/lib/meta-sosial.ts"
type: "code"
community: "BeritaLayout.astro"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/BeritaLayoutastro
---

# meta-sosial.ts

## Connections
- [[BaseLayout.astro]] - `imports_from` [EXTRACTED]
- [[BeritaLayout.astro]] - `imports_from` [EXTRACTED]
- [[MetaTag]] - `contains` [EXTRACTED]
- [[OgType]] - `contains` [EXTRACTED]
- [[PostDetail]] - `imports` [EXTRACTED]
- [[ROUTES]] - `imports` [EXTRACTED]
- [[ResolvedMedia]] - `imports` [EXTRACTED]
- [[SocialMeta]] - `contains` [EXTRACTED]
- [[n.astro]] - `imports_from` [EXTRACTED]
- [[slugindex.astro]] - `imports_from` [EXTRACTED]
- [[absoluteUrl()]] - `imports` [EXTRACTED]
- [[articleSocialMeta()]] - `contains` [EXTRACTED]
- [[berita.ts]] - `imports_from` [EXTRACTED]
- [[beritaslug.astro]] - `imports_from` [EXTRACTED]
- [[isHttpUrl()_1]] - `contains` [EXTRACTED]
- [[listingSocialMeta()]] - `contains` [EXTRACTED]
- [[media.ts]] - `imports_from` [EXTRACTED]
- [[meta-sosial.test.ts]] - `imports_from` [EXTRACTED]
- [[ogImageMeta()]] - `contains` [EXTRACTED]
- [[postSeoText()]] - `contains` [EXTRACTED]
- [[routes.ts]] - `imports_from` [EXTRACTED]
- [[rubrikPaginationLinks()]] - `contains` [EXTRACTED]
- [[site.ts]] - `imports_from` [EXTRACTED]
- [[twitterCardMeta()]] - `contains` [EXTRACTED]
- [[videoslug.astro]] - `imports_from` [EXTRACTED]
- [[videoSocialMeta()]] - `contains` [EXTRACTED]
- [[youtubeEmbedUrl()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/BeritaLayoutastro