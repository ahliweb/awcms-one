---
source_file: "apps/storefront/src/lib/awcms/media.ts"
type: "code"
community: "portable-text.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/portable-textts
---

# media.ts

## Connections
- [[AwcmsApiError]] - `imports` [EXTRACTED]
- [[BeritaLayout.astro]] - `imports_from` [EXTRACTED]
- [[MediaPublicOrigin]] - `contains` [EXTRACTED]
- [[RawResolvedMediaItem]] - `contains` [EXTRACTED]
- [[ResolvedMedia]] - `contains` [EXTRACTED]
- [[UNCONFIGURED]] - `contains` [EXTRACTED]
- [[awcms-media.test.ts]] - `imports_from` [EXTRACTED]
- [[awcmsGet()]] - `imports` [EXTRACTED]
- [[berita-portable-text.test.ts]] - `imports_from` [EXTRACTED]
- [[berita.ts]] - `imports_from` [EXTRACTED]
- [[chunk()]] - `contains` [EXTRACTED]
- [[client.ts]] - `imports_from` [EXTRACTED]
- [[csp.json.ts]] - `imports_from` [EXTRACTED]
- [[fetchMediaPublicOrigin()]] - `contains` [EXTRACTED]
- [[getMediaPublicOrigin()]] - `contains` [EXTRACTED]
- [[isExpectedRefusal()_3]] - `contains` [EXTRACTED]
- [[lembaga.ts]] - `imports_from` [EXTRACTED]
- [[logo-instansi.test.ts]] - `imports_from` [EXTRACTED]
- [[markUnresolved()]] - `contains` [EXTRACTED]
- [[meta-sosial.test.ts]] - `imports_from` [EXTRACTED]
- [[meta-sosial.ts]] - `imports_from` [EXTRACTED]
- [[portable-text.ts]] - `imports_from` [EXTRACTED]
- [[resetMediaCachesForTests()]] - `contains` [EXTRACTED]
- [[resolveMedia()]] - `contains` [EXTRACTED]
- [[resolveOneMedia()]] - `contains` [EXTRACTED]
- [[resolvedCache]] - `contains` [EXTRACTED]
- [[toResolvedMedia()]] - `contains` [EXTRACTED]
- [[unresolvedIds]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/portable-textts