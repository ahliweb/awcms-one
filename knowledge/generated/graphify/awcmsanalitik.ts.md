---
source_file: "apps/storefront/src/lib/awcms/analitik.ts"
type: "code"
community: "awcms/analitik.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/awcms/analitikts
---

# awcms/analitik.ts

## Connections
- [[AwcmsApiError]] - `imports` [EXTRACTED]
- [[Sidebar.astro]] - `imports_from` [EXTRACTED]
- [[TERPOPULER_RANGE]] - `contains` [EXTRACTED]
- [[TopPath]] - `contains` [EXTRACTED]
- [[analitik-terpopuler.test.ts]] - `imports_from` [EXTRACTED]
- [[awcmsGet()]] - `imports` [EXTRACTED]
- [[client.ts]] - `imports_from` [EXTRACTED]
- [[fetchTopPaths()]] - `contains` [EXTRACTED]
- [[getTopPaths()]] - `contains` [EXTRACTED]
- [[hitungTayangPerSlug()]] - `contains` [EXTRACTED]
- [[isExpectedRefusal()_2]] - `contains` [EXTRACTED]
- [[pilihTerpopuler()]] - `contains` [EXTRACTED]
- [[resetAnalitikCacheForTests()]] - `contains` [EXTRACTED]
- [[slugDariPath()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/awcms/analitikts