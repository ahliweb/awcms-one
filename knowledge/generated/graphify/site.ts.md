---
source_file: "apps/storefront/src/config/site.ts"
type: "code"
community: "Site Config, Sitemaps & Theme"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Site_Config_Sitemaps__Theme
---

# site.ts

## Connections
- [[ArtikelView.astro]] - `imports_from` [EXTRACTED]
- [[BaseLayout.astro]] - `imports_from` [EXTRACTED]
- [[DEFAULT_IDENTITY]] - `contains` [EXTRACTED]
- [[DEFAULT_THEME_COLORS]] - `contains` [EXTRACTED]
- [[slugfeed.xml.ts]] - `imports_from` [EXTRACTED]
- [[absoluteUrl()]] - `contains` [EXTRACTED]
- [[beritafeed.xml.ts]] - `imports_from` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[jsonld-berita.ts]] - `imports_from` [EXTRACTED]
- [[kategorislug.astro]] - `imports_from` [EXTRACTED]
- [[meta-sosial.ts]] - `imports_from` [EXTRACTED]
- [[pagesfeed.xml.ts]] - `imports_from` [EXTRACTED]
- [[productslug.astro]] - `imports_from` [EXTRACTED]
- [[profil.test.ts]] - `imports_from` [EXTRACTED]
- [[profil.ts]] - `imports_from` [EXTRACTED]
- [[readEnvOr()]] - `imports` [EXTRACTED]
- [[robots.txt.ts]] - `imports_from` [EXTRACTED]
- [[siteConfig]] - `contains` [EXTRACTED]
- [[siteUrl]] - `contains` [EXTRACTED]
- [[sitemap-index.xml.ts]] - `imports_from` [EXTRACTED]
- [[sitemap-katalog.ts]] - `imports_from` [EXTRACTED]
- [[sitemap-sources.ts]] - `imports_from` [EXTRACTED]
- [[theme.ts]] - `imports_from` [EXTRACTED]
- [[warna.test.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Site_Config_Sitemaps__Theme