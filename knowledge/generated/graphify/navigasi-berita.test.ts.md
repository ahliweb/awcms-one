---
source_file: "apps/storefront/tests/navigasi-berita.test.ts"
type: "code"
community: "navigasi-berita.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/navigasi-beritats
---

# navigasi-berita.test.ts

## Connections
- [[DAERAH_PANEL_ID]] - `imports` [EXTRACTED]
- [[ROUTES]] - `imports` [EXTRACTED]
- [[RawInstitution]] - `imports` [EXTRACTED]
- [[RegionRef]] - `imports` [EXTRACTED]
- [[RubrikNode]] - `imports` [EXTRACTED]
- [[berita.ts]] - `imports_from` [EXTRACTED]
- [[blog.ts]] - `imports_from` [EXTRACTED]
- [[institution()]] - `contains` [EXTRACTED]
- [[navigasi-berita.ts]] - `imports_from` [EXTRACTED]
- [[routes.ts]] - `imports_from` [EXTRACTED]
- [[rubrik()]] - `contains` [EXTRACTED]
- [[selectDaerahList()]] - `imports` [EXTRACTED]
- [[selectMitraOrder()]] - `imports` [EXTRACTED]
- [[selectNavUtamaRubrik()]] - `imports` [EXTRACTED]
- [[selectPerusahaanLinks()]] - `imports` [EXTRACTED]
- [[selectUmumChildren()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/navigasi-beritats