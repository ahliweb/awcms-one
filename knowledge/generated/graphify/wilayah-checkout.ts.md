---
source_file: "apps/storefront/src/lib/awcms/wilayah-checkout.ts"
type: "code"
community: "Build-Time CMS Client & Region Data"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Build-Time_CMS_Client__Region_Data
---

# wilayah-checkout.ts

## Connections
- [[ADR-0002_1]] - `cites` [EXTRACTED]
- [[ADR-0007_3]] - `cites` [EXTRACTED]
- [[ConcurrencyLimiter]] - `contains` [EXTRACTED]
- [[DEFAULT_PROVINCE_CODES]] - `contains` [EXTRACTED]
- [[MAX_IN_FLIGHT_REGION_REQUESTS]] - `contains` [EXTRACTED]
- [[RegionsPage_1]] - `contains` [EXTRACTED]
- [[WilayahRegion]] - `contains` [EXTRACTED]
- [[awcmsGet()]] - `imports` [EXTRACTED]
- [[client.ts]] - `imports_from` [EXTRACTED]
- [[configuredProvinceCodes()]] - `contains` [EXTRACTED]
- [[createConcurrencyLimiter()]] - `contains` [EXTRACTED]
- [[districtsCache]] - `contains` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[getAllCheckoutRegencies()]] - `contains` [EXTRACTED]
- [[getCheckoutDistricts()]] - `contains` [EXTRACTED]
- [[getCheckoutProvinces()]] - `contains` [EXTRACTED]
- [[getCheckoutRegencies()]] - `contains` [EXTRACTED]
- [[listRegions()_1]] - `contains` [EXTRACTED]
- [[matchesQuery()]] - `contains` [EXTRACTED]
- [[readEnv()]] - `imports` [EXTRACTED]
- [[regenciesCache_1]] - `contains` [EXTRACTED]
- [[regionRequestLimiter]] - `contains` [EXTRACTED]
- [[resetWilayahCheckoutCachesForTests()]] - `contains` [EXTRACTED]
- [[wilayah-checkout.test.ts]] - `imports_from` [EXTRACTED]
- [[wilayah-kabupaten-provinceCode.json.ts]] - `imports_from` [EXTRACTED]
- [[wilayah-kecamatan-cityCode.json.ts]] - `imports_from` [EXTRACTED]
- [[wilayah-provinsi.json.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Build-Time_CMS_Client__Region_Data