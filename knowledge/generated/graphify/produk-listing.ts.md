---
source_file: "apps/storefront/src/scripts/produk-listing.ts"
type: "code"
community: "produk-listing.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/produk-listingts
---

# produk-listing.ts

## Connections
- [[KNOWN_SORTS]] - `contains` [EXTRACTED]
- [[ListingState]] - `contains` [EXTRACTED]
- [[ProductSort]] - `imports` [EXTRACTED]
- [[ProdukIndexFilter]] - `imports` [EXTRACTED]
- [[applyAndRender()]] - `contains` [EXTRACTED]
- [[catalog.ts]] - `imports_from` [EXTRACTED]
- [[countEl]] - `contains` [EXTRACTED]
- [[emptyState_1]] - `contains` [EXTRACTED]
- [[fetchProdukIndex()]] - `imports` [EXTRACTED]
- [[filterProdukIndex()]] - `imports` [EXTRACTED]
- [[form]] - `contains` [EXTRACTED]
- [[grid_1]] - `contains` [EXTRACTED]
- [[isProductSort()]] - `contains` [EXTRACTED]
- [[paginateProdukIndex()]] - `imports` [EXTRACTED]
- [[paginationEl_1]] - `contains` [EXTRACTED]
- [[parseNumberParam()]] - `contains` [EXTRACTED]
- [[produk-index-klien.ts]] - `imports_from` [EXTRACTED]
- [[produk.astro]] - `imports_from` [EXTRACTED]
- [[readFormState()]] - `contains` [EXTRACTED]
- [[readStateFromUrl()]] - `contains` [EXTRACTED]
- [[render()_1]] - `contains` [EXTRACTED]
- [[renderPagination()]] - `imports` [EXTRACTED]
- [[renderProdukGrid()]] - `imports` [EXTRACTED]
- [[syncControlsToState()]] - `contains` [EXTRACTED]
- [[writeStateToUrl()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/produk-listingts