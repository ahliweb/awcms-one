---
source_file: "apps/storefront/tests/pengalihan-aturan.test.ts"
type: "code"
community: "penyaji.mjs"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/penyajimjs
---

# pengalihan-aturan.test.ts

## Connections
- [[DAERAH_NAMES]] - `imports` [EXTRACTED]
- [[MITRA_BORNEO_NAMES]] - `imports` [EXTRACTED]
- [[ROW_MAP]] - `contains` [EXTRACTED]
- [[ROW_MAP_SYNTHETIC_VIDEO_KEYS]] - `contains` [EXTRACTED]
- [[UMUM_NAMES]] - `imports` [EXTRACTED]
- [[legacyRedirectLocation()]] - `imports` [EXTRACTED]
- [[normalizeSlugSegment()]] - `imports` [EXTRACTED]
- [[ownKeys()]] - `contains` [EXTRACTED]
- [[pengalihan-aturan.mjs]] - `imports_from` [EXTRACTED]
- [[penyaji.mjs]] - `imports_from` [EXTRACTED]
- [[ruleBasedRedirectLocation()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/penyajimjs