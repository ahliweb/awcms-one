---
source_file: "apps/storefront/src/lib/navigasi-berita.ts"
type: "code"
community: "navigasi-berita.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/navigasi-beritats
---

# navigasi-berita.ts

## Connections
- [[BilahUtilitas.astro]] - `imports_from` [EXTRACTED]
- [[DAERAH_PANEL_ID]] - `contains` [EXTRACTED]
- [[DAERAH_URUTAN]] - `contains` [EXTRACTED]
- [[FooterBerita.astro]] - `imports_from` [EXTRACTED]
- [[MitraNavItem]] - `contains` [EXTRACTED]
- [[NavBerita.astro]] - `imports_from` [EXTRACTED]
- [[NavUtamaItem]] - `contains` [EXTRACTED]
- [[PERUSAHAAN_LINKS]] - `contains` [EXTRACTED]
- [[PerusahaanLink]] - `contains` [EXTRACTED]
- [[ROUTES]] - `imports` [EXTRACTED]
- [[RUBRIK_UTAMA]] - `contains` [EXTRACTED]
- [[RawInstitution]] - `imports` [EXTRACTED]
- [[RegionRecord]] - `imports` [EXTRACTED]
- [[RegionRef]] - `imports` [EXTRACTED]
- [[RubrikNode]] - `imports` [EXTRACTED]
- [[STATIC_PAGE_SLUGS]] - `imports` [EXTRACTED]
- [[Sidebar.astro]] - `imports_from` [EXTRACTED]
- [[UmumItem]] - `contains` [EXTRACTED]
- [[berita.ts]] - `imports_from` [EXTRACTED]
- [[blog.ts]] - `imports_from` [EXTRACTED]
- [[branchOrder()]] - `contains` [EXTRACTED]
- [[daerahOrderIndex()]] - `contains` [EXTRACTED]
- [[findKaltengProvince()]] - `imports` [EXTRACTED]
- [[flattenRubrikTree()]] - `imports` [EXTRACTED]
- [[getAllInstitutions()]] - `imports` [EXTRACTED]
- [[getDaerahList()]] - `contains` [EXTRACTED]
- [[getMitraList()_1]] - `contains` [EXTRACTED]
- [[getNavUtama()]] - `contains` [EXTRACTED]
- [[getPerusahaanLinks()]] - `contains` [EXTRACTED]
- [[getResolvableRegionsByCode()]] - `imports` [EXTRACTED]
- [[getRubrikTree()]] - `imports` [EXTRACTED]
- [[getUmumList()]] - `contains` [EXTRACTED]
- [[listStaticPages()]] - `imports` [EXTRACTED]
- [[navigasi-berita.test.ts]] - `imports_from` [EXTRACTED]
- [[normalizeForMatch()]] - `contains` [EXTRACTED]
- [[pages.ts]] - `imports_from` [EXTRACTED]
- [[routes.ts]] - `imports_from` [EXTRACTED]
- [[selectDaerahList()]] - `contains` [EXTRACTED]
- [[selectMitraOrder()]] - `contains` [EXTRACTED]
- [[selectNavUtamaRubrik()]] - `contains` [EXTRACTED]
- [[selectPerusahaanLinks()]] - `contains` [EXTRACTED]
- [[selectUmumChildren()]] - `contains` [EXTRACTED]
- [[stripRegionTerm()]] - `contains` [EXTRACTED]
- [[toRegionRef()]] - `imports` [EXTRACTED]
- [[wilayah.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/navigasi-beritats