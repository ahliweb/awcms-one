---
source_file: "apps/storefront/src/lib/akun-sesi.ts"
type: "code"
community: "Customer Session Store"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Customer_Session_Store
---

# akun-sesi.ts

## Connections
- [[AKUN_EVENT_NAME]] - `imports` [EXTRACTED]
- [[AKUN_STORAGE_KEY]] - `imports` [EXTRACTED]
- [[SesiAkun]] - `imports` [EXTRACTED]
- [[akun-afiliasi.ts]] - `imports_from` [EXTRACTED]
- [[akun-alamat.ts]] - `imports_from` [EXTRACTED]
- [[akun-header.ts]] - `imports_from` [EXTRACTED]
- [[akun-klien.test.ts]] - `imports_from` [EXTRACTED]
- [[akun-klien.ts]] - `imports_from` [EXTRACTED]
- [[akun-kontrak.ts]] - `imports_from` [EXTRACTED]
- [[akun-pesanan.ts]] - `imports_from` [EXTRACTED]
- [[akun-ulasan.ts]] - `imports_from` [EXTRACTED]
- [[akun.ts]] - `imports_from` [EXTRACTED]
- [[bacaSesi()]] - `contains` [EXTRACTED]
- [[checkout.ts]] - `imports_from` [EXTRACTED]
- [[daftar.ts]] - `imports_from` [EXTRACTED]
- [[hapusSesi()]] - `contains` [EXTRACTED]
- [[isSesiKedaluwarsa()]] - `imports` [EXTRACTED]
- [[masuk.ts]] - `imports_from` [EXTRACTED]
- [[parseSesi()]] - `imports` [EXTRACTED]
- [[simpanSesi()]] - `contains` [EXTRACTED]
- [[tulis()]] - `contains` [EXTRACTED]
- [[wishlist-akun-sync.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Customer_Session_Store