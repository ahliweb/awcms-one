---
source_file: "apps/storefront/src/scripts/akun-alamat.ts"
type: "code"
community: "akun-alamat.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/akun-alamatts
---

# akun-alamat.ts

## Connections
- [[AKUN_EVENT_NAME]] - `imports` [EXTRACTED]
- [[Alamat]] - `imports` [EXTRACTED]
- [[AlamatInput]] - `imports` [EXTRACTED]
- [[TokoApiError]] - `imports` [EXTRACTED]
- [[akun-klien.ts]] - `imports_from` [EXTRACTED]
- [[akun-kontrak.ts]] - `imports_from` [EXTRACTED]
- [[akun-sesi.ts]] - `imports_from` [EXTRACTED]
- [[alamat.astro]] - `imports_from` [EXTRACTED]
- [[ambilAlamat()]] - `imports` [EXTRACTED]
- [[applyRegionSelection()]] - `imports` [EXTRACTED]
- [[bacaSesi()]] - `imports` [EXTRACTED]
- [[buildWhatsappAccountMessage()]] - `imports` [EXTRACTED]
- [[buildWhatsappUrl()]] - `imports` [EXTRACTED]
- [[clearFieldErrors()]] - `contains` [EXTRACTED]
- [[closeForm()]] - `indirect_call` [INFERRED]
- [[deleteAlamat()]] - `contains` [EXTRACTED]
- [[hapusAlamat()]] - `imports` [EXTRACTED]
- [[hideSubmitError()_1]] - `contains` [EXTRACTED]
- [[jadikanAlamatUtama()]] - `imports` [EXTRACTED]
- [[loadList()]] - `contains` [EXTRACTED]
- [[openFormForCreate()]] - `indirect_call` [INFERRED]
- [[openFormForEdit()]] - `contains` [EXTRACTED]
- [[render()_3]] - `indirect_call` [INFERRED]
- [[renderList()_1]] - `contains` [EXTRACTED]
- [[root_6]] - `contains` [EXTRACTED]
- [[setDefault()]] - `contains` [EXTRACTED]
- [[showAccountView()_1]] - `contains` [EXTRACTED]
- [[showGuestView()_1]] - `contains` [EXTRACTED]
- [[showSubmitError()_1]] - `contains` [EXTRACTED]
- [[tambahAlamat()]] - `imports` [EXTRACTED]
- [[toko-permintaan.ts]] - `imports_from` [EXTRACTED]
- [[ubahAlamat()]] - `imports` [EXTRACTED]
- [[wa-fallback.ts]] - `imports_from` [EXTRACTED]
- [[wilayah-region-select.ts]] - `imports_from` [EXTRACTED]
- [[wireCascadingRegionSelects()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/akun-alamatts