---
source_file: "apps/storefront/src/scripts/produk-index-klien.ts"
type: "code"
community: "produk-listing.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/produk-listingts
---

# produk-index-klien.ts

## Connections
- [[ProdukIndexEntry]] - `imports` [EXTRACTED]
- [[cardHtml()]] - `contains` [EXTRACTED]
- [[cari-listing.ts]] - `imports_from` [EXTRACTED]
- [[catalog.ts]] - `imports_from` [EXTRACTED]
- [[escapeHtml()]] - `contains` [EXTRACTED]
- [[fetchProdukIndex()]] - `contains` [EXTRACTED]
- [[formatPrice()]] - `imports` [EXTRACTED]
- [[harga.ts]] - `imports_from` [EXTRACTED]
- [[labelClassName()]] - `imports` [EXTRACTED]
- [[produk-listing.ts]] - `imports_from` [EXTRACTED]
- [[renderPagination()]] - `contains` [EXTRACTED]
- [[renderProdukGrid()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/produk-listingts