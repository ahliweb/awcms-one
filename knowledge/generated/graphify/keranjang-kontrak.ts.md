---
source_file: "apps/storefront/src/lib/keranjang-kontrak.ts"
type: "code"
community: "keranjang-kontrak.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/keranjang-kontrakts
---

# keranjang-kontrak.ts

## Connections
- [[ADR-0003_1]] - `cites` [EXTRACTED]
- [[Cart]] - `contains` [EXTRACTED]
- [[CartLine]] - `contains` [EXTRACTED]
- [[KERANJANG_EVENT_NAME]] - `contains` [EXTRACTED]
- [[KERANJANG_STORAGE_KEY]] - `contains` [EXTRACTED]
- [[addOrMergeLine()]] - `contains` [EXTRACTED]
- [[checkout.ts]] - `imports_from` [EXTRACTED]
- [[countCartItems()]] - `contains` [EXTRACTED]
- [[createEmptyCart()]] - `contains` [EXTRACTED]
- [[isIsoDateString()]] - `contains` [EXTRACTED]
- [[isSameConfiguration()]] - `contains` [EXTRACTED]
- [[katalog-keranjang.test.ts]] - `imports_from` [EXTRACTED]
- [[keranjang-hitung.ts]] - `imports_from` [EXTRACTED]
- [[keranjang-klien.ts]] - `imports_from` [EXTRACTED]
- [[keranjang.ts]] - `imports_from` [EXTRACTED]
- [[parseCart()]] - `contains` [EXTRACTED]
- [[removeLine()]] - `contains` [EXTRACTED]
- [[setLineQuantity()]] - `contains` [EXTRACTED]
- [[validateCartLine()]] - `contains` [EXTRACTED]
- [[wa-fallback.test.ts]] - `imports_from` [EXTRACTED]
- [[wa-fallback.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/keranjang-kontrakts