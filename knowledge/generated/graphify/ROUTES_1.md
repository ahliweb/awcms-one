---
source_file: "apps/storefront/src/config/routes.ts"
type: "code"
community: "routes.ts"
location: "L17"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/routests
---

# ROUTES

## Connections
- [[ArtikelView.astro]] - `imports` [EXTRACTED]
- [[CategoryTree.astro]] - `imports` [EXTRACTED]
- [[Footer.astro]] - `imports` [EXTRACTED]
- [[FooterBerita.astro]] - `imports` [EXTRACTED]
- [[HalamanDepanBerita.astro]] - `imports` [EXTRACTED]
- [[Header.astro]] - `imports` [EXTRACTED]
- [[LogoInstansi.astro]] - `imports` [EXTRACTED]
- [[NavBerita.astro]] - `imports` [EXTRACTED]
- [[RubrikBody.astro]] - `imports` [EXTRACTED]
- [[Sidebar.astro]] - `imports` [EXTRACTED]
- [[slugfeed.xml.ts]] - `imports` [EXTRACTED]
- [[slugindex.astro]] - `imports` [EXTRACTED]
- [[akun-header.ts]] - `imports` [EXTRACTED]
- [[akun-pesanan.ts]] - `imports` [EXTRACTED]
- [[akunindex.astro]] - `imports` [EXTRACTED]
- [[akunpesanan.astro]] - `imports` [EXTRACTED]
- [[berita-guard-no-news-route.test.ts]] - `imports` [EXTRACTED]
- [[berita-routes.test.ts]] - `imports` [EXTRACTED]
- [[beritaslug.astro]] - `imports` [EXTRACTED]
- [[beritafeed.xml.ts]] - `imports` [EXTRACTED]
- [[beritaindex.astro]] - `imports` [EXTRACTED]
- [[beritapagesarsipyyyymm.astro]] - `imports` [EXTRACTED]
- [[beritapagescari-berita.astro]] - `imports` [EXTRACTED]
- [[beritapagesrubrikslughalamann.astro]] - `imports` [EXTRACTED]
- [[configprofil.ts]] - `imports` [EXTRACTED]
- [[daerahslug.astro]] - `imports` [EXTRACTED]
- [[daftar.ts]] - `imports` [EXTRACTED]
- [[kategorislug.astro]] - `imports` [EXTRACTED]
- [[landingBeranda.astro]] - `imports` [EXTRACTED]
- [[masuk.ts]] - `imports` [EXTRACTED]
- [[meta-sosial.ts]] - `imports` [EXTRACTED]
- [[mitraslug.astro]] - `imports` [EXTRACTED]
- [[navigasi-berita.test.ts]] - `imports` [EXTRACTED]
- [[navigasi-berita.ts]] - `imports` [EXTRACTED]
- [[navigasi-profil.ts]] - `imports` [EXTRACTED]
- [[pengalihan-legacy.ts]] - `imports` [EXTRACTED]
- [[penulisslug.astro]] - `imports` [EXTRACTED]
- [[pesan.astro]] - `imports` [EXTRACTED]
- [[productslug.astro]] - `imports` [EXTRACTED]
- [[profil-build-smoke.test.ts]] - `imports` [EXTRACTED]
- [[profil-konfig.test.ts]] - `imports` [EXTRACTED]
- [[routes.test.ts]] - `imports` [EXTRACTED]
- [[routes.ts]] - `contains` [EXTRACTED]
- [[sitemap-katalog.ts]] - `imports` [EXTRACTED]
- [[sitemap-sources.ts]] - `imports` [EXTRACTED]
- [[tagslug.astro]] - `imports` [EXTRACTED]
- [[tokoBeranda.astro]] - `imports` [EXTRACTED]
- [[tokopagesakunafiliasi.astro]] - `imports` [EXTRACTED]
- [[tokopagesakunalamat.astro]] - `imports` [EXTRACTED]
- [[tokopagesakunulasan.astro]] - `imports` [EXTRACTED]
- [[tokopagesdaftar.astro]] - `imports` [EXTRACTED]
- [[tokopagesmasuk.astro]] - `imports` [EXTRACTED]
- [[videoslug.astro]] - `imports` [EXTRACTED]
- [[videoindex.astro]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/routests