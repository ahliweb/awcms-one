---
source_file: "apps/storefront/src/lib/awcms/theme.ts"
type: "code"
community: "Theme Token Fetch"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Theme_Token_Fetch
---

# theme.ts

## Connections
- [[ADR-0009]] - `cites` [EXTRACTED]
- [[DEFAULT_THEME_COLORS]] - `imports` [EXTRACTED]
- [[ThemeColors]] - `contains` [EXTRACTED]
- [[apiOrigin()]] - `contains` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[extractThemeToken()]] - `contains` [EXTRACTED]
- [[fetchSiteTheme()]] - `contains` [EXTRACTED]
- [[getSiteTheme()]] - `contains` [EXTRACTED]
- [[manifest.webmanifest.ts]] - `imports_from` [EXTRACTED]
- [[readEnv()]] - `imports` [EXTRACTED]
- [[resetSiteThemeCacheForTests()]] - `contains` [EXTRACTED]
- [[site.ts]] - `imports_from` [EXTRACTED]
- [[tenantCode()]] - `contains` [EXTRACTED]
- [[theme-tokens.css.ts]] - `imports_from` [EXTRACTED]
- [[theme.test.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Theme_Token_Fetch