---
source_file: "apps/storefront/src/scripts/pesanan.ts"
type: "code"
community: "scripts/pesanan.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/scripts/pesanants
---

# scripts/pesanan.ts

## Connections
- [[Order]] - `imports` [EXTRACTED]
- [[PESANAN_PHONE_KEY]] - `imports` [EXTRACTED]
- [[STATUS_LABELS]] - `contains` [EXTRACTED]
- [[TokoApiError]] - `imports` [EXTRACTED]
- [[cancelOrder()]] - `imports` [EXTRACTED]
- [[formatPrice()]] - `imports` [EXTRACTED]
- [[getOrder()]] - `imports` [EXTRACTED]
- [[harga.ts]] - `imports_from` [EXTRACTED]
- [[loadOrder()]] - `contains` [EXTRACTED]
- [[pesanan-sesi.ts]] - `imports_from` [EXTRACTED]
- [[pesanan.astro]] - `imports_from` [EXTRACTED]
- [[readStoredPhone()]] - `contains` [EXTRACTED]
- [[renderCountdown()]] - `contains` [EXTRACTED]
- [[renderLines()_1]] - `contains` [EXTRACTED]
- [[renderOrder()]] - `contains` [EXTRACTED]
- [[renderPaymentInstructions()]] - `contains` [EXTRACTED]
- [[renderSummary()_1]] - `contains` [EXTRACTED]
- [[renderTimeline()]] - `contains` [EXTRACTED]
- [[root_5]] - `contains` [EXTRACTED]
- [[storePhone()]] - `contains` [EXTRACTED]
- [[submitPaymentConfirmation()]] - `imports` [EXTRACTED]
- [[toko-klien.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/scripts/pesanants