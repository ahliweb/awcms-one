---
source_file: "apps/storefront/tests/analitik-terpopuler.test.ts"
type: "code"
community: "awcms/analitik.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/awcms/analitikts
---

# analitik-terpopuler.test.ts

## Connections
- [[AwcmsApiError]] - `imports` [EXTRACTED]
- [[TERPOPULER_RANGE]] - `imports` [EXTRACTED]
- [[TopPath]] - `imports` [EXTRACTED]
- [[awcmsanalitik.ts]] - `imports_from` [EXTRACTED]
- [[client.ts]] - `imports_from` [EXTRACTED]
- [[getTopPaths()]] - `imports` [EXTRACTED]
- [[hitungTayangPerSlug()]] - `imports` [EXTRACTED]
- [[jsonResponse()]] - `contains` [EXTRACTED]
- [[mockFetch()_2]] - `contains` [EXTRACTED]
- [[newestFirst]] - `contains` [EXTRACTED]
- [[pilihTerpopuler()]] - `imports` [EXTRACTED]
- [[post()]] - `contains` [EXTRACTED]
- [[resetAnalitikCacheForTests()]] - `imports` [EXTRACTED]
- [[slugDariPath()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/awcms/analitikts