---
source_file: "apps/storefront/src/scripts/produk-detail.ts"
type: "code"
community: "produk-detail.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/produk-detailts
---

# produk-detail.ts

## Connections
- [[CommerceProductVariant]] - `imports` [EXTRACTED]
- [[FlashSalePayload]] - `contains` [EXTRACTED]
- [[ProdukDetailPayload]] - `contains` [EXTRACTED]
- [[ServiceFormFieldPayload]] - `contains` [EXTRACTED]
- [[addToCart()]] - `imports` [EXTRACTED]
- [[catalog.ts]] - `imports_from` [EXTRACTED]
- [[currentVariant()]] - `contains` [EXTRACTED]
- [[effectiveMaxQuantity()]] - `contains` [EXTRACTED]
- [[effectivePrice()]] - `contains` [EXTRACTED]
- [[findVariantForSelection()]] - `imports` [EXTRACTED]
- [[formatPrice()]] - `imports` [EXTRACTED]
- [[harga.ts]] - `imports_from` [EXTRACTED]
- [[hasSelection()]] - `contains` [EXTRACTED]
- [[keranjang-klien.ts]] - `imports_from` [EXTRACTED]
- [[productslug.astro]] - `imports_from` [EXTRACTED]
- [[refresh()_1]] - `indirect_call` [INFERRED]
- [[root_6]] - `contains` [EXTRACTED]
- [[selectedOptionNames()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/produk-detailts