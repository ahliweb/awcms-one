...
<!-- i18n-source-hash: sha256:36bff81f983cfc2f8c711ba46c69b605a64113ae773381703777ab862c45110c -->
...
## Gerbang pembayaran: Midtrans Snap (issue #110, contract #106 D3)
...
- **Lookup bootstrap webhook-endpoint.** `awcms_resolve_commerce_webhook_endpoint(token_hash)` adalah fungsi `SECURITY DEFINER` yang meniru pola `awcms_resolve_tenant_domain_lookup` persis (peran pemilik NOLOGIN khusus, policy baca yang dibatasi sempit, EXECUTE dibatasi ke `awcms_app`) — cara rute INTAKE webhook (belum dibangun) akan me-resolve `(tenant_id, provider)` dari token opak sebelum konteks tenant apa pun ada.
- **Sengaja di luar cakupan di sini (issue #113).** Rute INTAKE webhook itu sendiri (`POST /api/v1/commerce/webhooks/midtrans/{token}`), `markOrderPaidBySystem`, dan job `commerce:payments:reconcile` yang mem-poll sesi yang masih pending. Issue ini hanya mengirimkan skema, adapter provider, jalur pembuatan sesi, dan siklus hidup token webhook-endpoint.
...
- **Tarif kurir RajaOngkir dan gerbang pembayaran Midtrans Snap sudah sama-sama selesai** — lihat "Tarif kurir" (issue #107) dan "Gerbang pembayaran" (issue #110) di atas. Rute INTAKE webhook gerbang itu sendiri, `markOrderPaidBySystem`, dan job reconcile tetap issue #113.
