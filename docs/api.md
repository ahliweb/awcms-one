...
### External providers — payment gateway, courier, WhatsApp, POS, reports, inbox, campaigns (increment 5 — #107–#118; courier rates and WhatsApp DONE)

[ADR-0017](adr/0017-external-providers-are-commerce-owned-ports-with-env-credentials-and-token-addressed-webhooks.md) records the ten decisions (D1–D10) this surface is designed against, and [issue #106](https://github.com/ahliweb/awcms-one/issues/106) is where the OpenAPI shape lives, exactly as ADR-0016/#86 did for accounts. Courier rates (D4, [issue #107](https://github.com/ahliweb/awcms-one/issues/107)) is now implemented — see "Courier rates" above and [`docs/cms.md`](cms.md). **D5 (WhatsApp) is implemented too — see its own subsection immediately below.** No handler exists yet for any row in the table below — each is named in `ROUTE_PARITY_EXEMPTIONS` (`apps/cms/scripts/api-spec-check.ts`) with the child issue that removes it.
