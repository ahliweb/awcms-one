...
### External providers — payment gateway, courier, WhatsApp, POS, reports, inbox, campaigns (increment 5 — #107–#118; courier rates, WhatsApp, inbox, and campaigns DONE)

[ADR-0017](adr/0017-external-providers-are-commerce-owned-ports-with-env-credentials-and-token-addressed-webhooks.md) records the ten decisions (D1–D10) this surface is designed against, and [issue #106](https://github.com/ahliweb/awcms-one/issues/106) is where the OpenAPI shape lives, exactly as ADR-0016/#86 did for accounts. Courier rates (D4, [issue #107](https://github.com/ahliweb/awcms-one/issues/107)) is now implemented — see "Courier rates" above and [`docs/cms.md`](cms.md). **D5 (WhatsApp), D8 (inbox), and D9 (campaigns/consent) are implemented too — see their own subsections immediately below.** No handler exists yet for any row in the table below — each is named in `ROUTE_PARITY_EXEMPTIONS` (`apps/cms/scripts/api-spec-check.ts`) with the child issue that removes it.
...
On EXISTING, already-implemented paths (additive fields, no new exemption needed for these): `POST cart/quote`/`POST orders`' `destination`/`shipping.serviceId` for a real courier rate (D4, RajaOngkir) are **done** (#107, see above); still planned is `payment.method: "gateway"` (D3 — #110); `GET`/`PATCH account/me` gains `marketingConsent` — **done** (D9 — #114, see "Customer campaigns" below); `store-settings`'s `shipping.courier` is **done** (#107), `payment.gateway` is still planned, and `store-settings/public` gains `payment.gatewayEnabled` (D10 — #118). `POST account/otp/request`'s `via?: "email"|"whatsapp"` (D5) is no longer planned — it shipped in #108, see below.
