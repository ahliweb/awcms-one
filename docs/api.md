...
### External providers — payment gateway, courier, WhatsApp, POS, reports, inbox, campaigns (increment 5 — planned — #107–#118)

[ADR-0017](adr/0017-external-providers-are-commerce-owned-ports-with-env-credentials-and-token-addressed-webhooks.md) records the ten decisions (D1–D10) this surface is designed against, and [issue #106](https://github.com/ahliweb/awcms-one/issues/106) is where the OpenAPI shape lives, exactly as ADR-0016/#86 did for accounts. No handler exists yet for any row below — each is named in `ROUTE_PARITY_EXEMPTIONS` (`apps/cms/scripts/api-spec-check.ts`) with the child issue that removes it.
...
Also planned on EXISTING, already-implemented paths (additive fields, no new exemption needed): `POST cart/quote`/`POST orders` gain `destination`/`shipping.serviceId` for a real courier rate (D4, RajaOngkir — #107) and `payment.method: "gateway"` (D3 — #110); `POST account/otp/request` gains `via?: "email"|"whatsapp"` (D5 — #108); `GET`/`PATCH account/me` gains `marketingConsent` (D9 — #115); `store-settings` gains `shipping.courier`/`payment.gateway`, and `store-settings/public` gains `payment.gatewayEnabled` (D10 — #118).
