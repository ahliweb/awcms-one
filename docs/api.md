...
On EXISTING, already-implemented paths (additive fields, no new exemption needed for these): `POST cart/quote`/`POST orders`' `destination`/`shipping.serviceId` for a real courier rate (D4, RajaOngkir) are **done** (#107, see above); `payment.method: "gateway"` (D3) is **done** too (#110, see below). `GET`/`PATCH account/me` gains `marketingConsent` — **done** (D9 — #114, see "Customer campaigns" below); `store-settings`'s `shipping.courier` and `payment.gateway` are both **done** (#107, #110), and `store-settings/public` gains `payment.gatewayEnabled` (**done**, #110) — only `shipping.courierEnabled`'s BjekMart-Features-flag sibling (D10, #118) is still planned. `POST account/otp/request`'s `via?: "email"|"whatsapp"` (D5) is no longer planned — it shipped in #108, see below.
