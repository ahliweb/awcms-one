...
### External providers — courier, payment gateway, WhatsApp, inbox DONE; POS, reports, campaigns still planned (increment 5 — #107–#118)

[ADR-0017](adr/0017-external-providers-are-commerce-owned-ports-with-env-credentials-and-token-addressed-webhooks.md) records the ten decisions (D1–D10) this surface is designed against, and [issue #106](https://github.com/ahliweb/awcms-one/issues/106) is where the OpenAPI shape lives, exactly as ADR-0016/#86 did for accounts. Courier rates (D4, [issue #107](https://github.com/ahliweb/awcms-one/issues/107)) is now implemented — see "Courier rates" above and [`docs/cms.md`](cms.md). **D5 (WhatsApp), D8 (inbox), and the D2/D3 session half of the payment gateway (issue #110) are implemented too — see their own subsections immediately below.** No handler exists yet for any row in the table below — each is named in `ROUTE_PARITY_EXEMPTIONS` (`apps/cms/scripts/api-spec-check.ts`) with the child issue that removes it.

| Method                | Path                                                                         | Auth                                           | Notes                                                                                                                                                |
| --------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST`                | `webhooks/midtrans/{endpointToken}`                                          | none (public)                                  | Tenant resolved from the opaque token via `awcms_resolve_commerce_webhook_endpoint` (D2, the lookup function landed in #110); always `200` for a verified/deduplicated/replayed event, `401` bad signature, `404` unknown token — #113 |
...
On EXISTING, already-implemented paths (additive fields, no new exemption needed for these): `POST cart/quote`/`POST orders`' `destination`/`shipping.serviceId` for a real courier rate (D4, RajaOngkir) are **done** (#107, see above); `payment.method: "gateway"` (D3) is **done** too (#110, see below). `GET`/`PATCH account/me` gains `marketingConsent` (D9 — #115); `store-settings`'s `shipping.courier` and `payment.gateway` are both **done** (#107, #110), and `store-settings/public` gains `payment.gatewayEnabled` (**done**, #110) — only `shipping.courierEnabled`'s BjekMart-Features-flag sibling (D10, #118) is still planned. `POST account/otp/request`'s `via?: "email"|"whatsapp"` (D5) is no longer planned — it shipped in #108, see below.

### Payment gateway — Midtrans Snap session half, implemented (#110, contract #106/ADR-0017 D3; the webhook INTAKE half is #113)

`PaymentGatewayProvider` (D3's port) is implemented by a Midtrans Snap adapter and a `log` dev/CI adapter, resolved from `COMMERCE_PAYMENT_GATEWAY` — see "Payment gateway" in [`docs/cms.md`](cms.md) for the full transaction-discipline/idempotency story.
