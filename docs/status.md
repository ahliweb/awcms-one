...
- CodeQL and the Playwright e2e/accessibility/responsive workflow as required status checks — both run on every push today but have not yet accumulated the green track record `.github/workflows/template-init-smoke.yml`'s own comment describes as the promotion path.
