...
**Idempotency:** running `template:init` a second time with the exact same flags is a no-op — exit **`0`**, printing "nothing to do." Running it again with one or more different flags rewrites again, touching only what actually changed. This is what makes it safe for [#138](https://github.com/ahliweb/awcms-one/issues/138)'s own `local-ci/template-*` legs (`bun run ci:template`) to run unattended, and safe for a human to re-run after fixing a typo in an earlier flag.
...
**A derived repository starts with no root GitHub Actions workflows, by design (issue #225, ADR-0021).** `template:init` never generates or references anything under `.github/workflows/`, under any flag — `awcms-one` itself runs zero GitHub Actions, and a repository created from it inherits that starting point rather than a workflow file that would need rewriting for a new owner/repo anyway. The gate chain `tools/ci/` runs (`bun run ci`, `bun run ci:pr -- <pr-number>`, `bun run ci:watch`) works unmodified in a derived repository the same way it does here — nothing in it is `awcms-one`-specific beyond the GitHub owner/repo `tools/ci/lib/github.ts` resolves from `git remote get-url origin` (or `LOCAL_CI_GITHUB_OWNER`/`LOCAL_CI_GITHUB_REPO`) — so running `bun run ci` locally, or standing up the same polling/status-reporting setup on the derived repository's own infrastructure, is the default path. A derived repository's own maintainers may instead attach a different, approved CI system, but that is their own explicit governance decision to make for their own repository, not something this template chooses for them or reintroduces on their behalf.
...
`TEMPLATE_INIT_TEST_SCOPE=root` (environment variable, local-CI-only) makes that trailing `bun test` run only the root gate tests (`bun test ./tests/` — the leading `./` matters, see below) instead of the whole workspace: `bun run ci:template`'s `local-ci/template-*` legs (formerly the `template-init-smoke` workflow) and `tests/template-init.test.mjs` both already run the full suite around the tool, and a nested full run doubled the stub-CMS build load enough to trip the storefront smoke tests' stub-start deadline. A real derived repository never sets it.
...
| 20 September 2026 | [#138](https://github.com/ahliweb/awcms-one/issues/138) | `bun run template:init` (`tools/template-init.ts` + `tools/template-init/**`), tested by `tests/template-init.test.mjs`, matrixed in CI by `.github/workflows/template-init-smoke.yml` (since issue #225, `bun run ci:template`'s `local-ci/template-*` legs). Its removal step and its `apps/storefront/.env.example` rewrite target #137's and #139's own landed layouts directly: `template:init --profil <p>` uncomments and sets the real `SITE_PROFILE` line #137 added, so a derived repository's build genuinely filters pages by the chosen profile from its very first build |
