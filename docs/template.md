...
- `tests/seed-profil.test.mjs` — [#139](https://github.com/ahliweb/awcms-one/issues/139)'s own test asserts BjekMart's reference example and the deprecation shim both exist and validate; both are removed by this same run, so `template:init` removes this file too rather than leaving a gate that fails on a tree it was never written to describe. Its other coverage (the `toko`/`berita`/`landing` seed schemas) is `tools/seed-data`'s own CI upkeep for the template, not something a derived deployment's contributions need to keep green.
