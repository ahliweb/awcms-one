...
<!-- i18n-source-hash: sha256:b52997409b63cb2f9c7facbb4b58b47da682fbbe6d929227f103d54ea2453c92 -->
...
Pada path YANG SUDAH ADA dan sudah diimplementasi (field aditif, tidak butuh exemption baru untuk yang ini): `destination`/`shipping.serviceId` milik `POST cart/quote`/`POST orders` untuk tarif kurir sungguhan (D4, RajaOngkir) **sudah selesai** (#107, lihat di atas); yang masih direncanakan adalah `payment.method: "gateway"` (D3 — #110); `GET`/`PATCH account/me` mendapat `marketingConsent` (D9 — #115); `shipping.courier` milik `store-settings` **sudah selesai** (#107), `payment.gateway` masih direncanakan, dan `store-settings/public` mendapat `payment.gatewayEnabled` (D10 — #118). `via?: "email"|"whatsapp"` pada `POST account/otp/request` (D5) tidak lagi direncanakan — sudah dikirim di #108, lihat di bawah.
