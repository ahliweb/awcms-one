...
<!-- i18n-source-hash: sha256:d0116a8b0767e2390685f46cc6aa5d6107327f0abc1c977c96797c8a06b40cbf -->
...
Pada path YANG SUDAH ADA dan sudah diimplementasi (field aditif, tidak butuh exemption baru untuk yang ini): `destination`/`shipping.serviceId` milik `POST cart/quote`/`POST orders` untuk tarif kurir sungguhan (D4, RajaOngkir) **sudah selesai** (#107, lihat di atas); yang masih direncanakan adalah `payment.method: "gateway"` (D3 — #110); `GET`/`PATCH account/me` mendapat `marketingConsent` — **sudah selesai** (D9 — #114, lihat "Kampanye pelanggan" di bawah); `shipping.courier` milik `store-settings` **sudah selesai** (#107), `payment.gateway` masih direncanakan, dan `store-settings/public` mendapat `payment.gatewayEnabled` (D10 — #118). `via?: "email"|"whatsapp"` pada `POST account/otp/request` (D5) tidak lagi direncanakan — sudah dikirim di #108, lihat di bawah.
