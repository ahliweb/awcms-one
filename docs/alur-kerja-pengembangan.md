...
`.github/dependabot.yml` (issue #180) opens version-update pull requests — separately from Dependabot *security* updates, which were already on (see "Branch protection on `main`" above) and cover any ecosystem regardless of this file. Two `updates` blocks:
...
- **`bun`, `directory: "/"`, `exclude-paths: ["apps/cms/**"]`.** This repo's own workspace dependencies (root `package.json`, `apps/storefront`, `packages/*`) against `bun.lock`. `apps/cms/**` is excluded because `apps/cms` is `ahliweb/awcms` embedded via `git subtree` (AGENTS.md's "The subtree embed") — upstream owns its own dependency set and ships its own `apps/cms/.github/dependabot.yml` (inert in this repo: GitHub only reads the config file at the repository root), and a version-update PR opened here against `apps/cms/package.json` would edit code a future `git subtree pull` can silently overwrite or conflict with. `exclude-paths` is a per-`updates`-block option that applies to every ecosystem, `bun` included. Same monthly/grouped shape as `github-actions`, commit prefix `chore(deps)`. Bun's own version is excluded from updates entirely (`ignore: [{dependency-name: "bun"}]`): it is pinned in three places that must move together (AGENTS.md's "Configuration and toolchain"), and a Dependabot PR only ever touches one dependency line, so raising it stays a deliberate, all-three-at-once change a maintainer makes by hand.

**Reviewing a Dependabot PR against the lockfile gate:** the same as any other dependency bump — `bun run check:lockfile` must still pass (it re-checks `bun.lock` against every workspace's `package.json`, so a Dependabot PR that only touched `bun.lock` without a matching `package.json` line, or the reverse, fails it exactly like a hand-edited lockfile would), then the root `bun test`/`audit:*` gates. A `github-actions` PR moves the SHA and its trailing `# vX.Y.Z` comment together — Dependabot has updated version comments after SHA-pinned actions since October 2022 — so the thing worth a human glance is that the comment still ends in the version (a comment with trailing prose after the version is the one shape Dependabot's own updater skips, per its docs), not that the update happened at all.
