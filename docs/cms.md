...
## Payment gateway: Midtrans Snap (issue #110, contract #106 D3)
...
- **The webhook-endpoint bootstrap lookup.** `awcms_resolve_commerce_webhook_endpoint(token_hash)` is a `SECURITY DEFINER` function mirroring `awcms_resolve_tenant_domain_lookup`'s pattern exactly (a dedicated NOLOGIN owner role, a narrowly scoped read policy, EXECUTE restricted to `awcms_app`) — how the (not-yet-built) webhook INTAKE route will resolve `(tenant_id, provider)` from an opaque token before any tenant context exists.
- **Deliberately out of scope here (issue #113).** The webhook INTAKE route itself (`POST /api/v1/commerce/webhooks/midtrans/{token}`), `markOrderPaidBySystem`, and the `commerce:payments:reconcile` job that polls pending sessions. This issue ships the schema, the provider adapters, the session-creation path, and the webhook-endpoint token lifecycle only.
...
- **RajaOngkir courier rates, the Midtrans Snap payment gateway, and the sales reports are all done** — see "Courier rates" (issue #107), "Payment gateway" (issue #110) and "Sales reports" (issue #117) above. The gateway's own webhook INTAKE route, `markOrderPaidBySystem`, and the reconcile job remain issue #113.
