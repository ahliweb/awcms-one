...
- **No WhatsApp/SMS OTP, e-mail/phone change on an existing account, phone verification, or tiered pricing applied at quote time** — [ADR-0016](adr/0016-customer-accounts-are-otp-verified-commerce-accounts-with-bearer-sessions.md) D6, all deferred as strict additions to [issue #32](https://github.com/ahliweb/awcms-one/issues/32)'s wave-0 contract. Customer accounts, OTP login, bearer sessions, the account dashboard (addresses/wishlist/orders/reviews), and the affiliate program are done — see "Customer account …" sections above.
