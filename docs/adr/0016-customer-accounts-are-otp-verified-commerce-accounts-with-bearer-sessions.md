...
**Status — 2026-09-19 (D6's tiered-pricing-at-quote-time follow-up, closed by Issue #118, contract #106/ADR-0017 D10):** `domain/cart-quote.ts`'s `quoteCart` now accepts an optional `customerLevel` (1–4) and prices a line at `price_level_{n}` (falling back to `price` when unset), resolved from an OPTIONAL bearer on `POST .../storefront/cart/quote` and re-applied, from the SAME account, inside `POST .../orders`' own re-quote — see [`docs/cms.md`](../cms.md#feature-toggles-and-tiered-pricing-issue-118-epic-33-c9-contract-106-d10-adr-0016-d6) and `apps/cms/src/modules/commerce/README.md`'s "Feature toggles & tiered pricing" section for the full design. D6's remaining three follow-ups — phone verification and e-mail/phone change on an existing account — are still open; WhatsApp OTP was already closed above (Issue #108).
