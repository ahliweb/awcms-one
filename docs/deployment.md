...
`bun run build`/`check`/`dev`/`serve` all honour `SITE_PROFILE ∈ {toko, berita, landing}` (default `toko`), read once at build time by `apps/storefront/src/config/profil.ts` — see [`docs/routing.md`](routing.md)'s "Build profiles" section for the mechanism and [`docs/template.md`](template.md) for the full matrix. Deploying a `berita`-only or `landing`-only site is the same two-step build-then-serve process above, with `SITE_PROFILE` set in the build environment; nothing about serving the resulting `dist/` differs — `apps/storefront/server/penyaji.mjs` derives what it can serve from the build it was given, the same way it already does for the CSP artifact. Local CI proves all three build (`bun run ci`'s `local-ci/check-toko|berita|landing` legs, all three required status checks on `main` alongside `local-ci/check-cms`), and the four `local-ci/template-*` legs (formerly `.github/workflows/template-init-smoke.yml`) additionally build each profile after a real `bun run template:init` run — all required status checks too — see [`docs/alur-kerja-pengembangan.md`](alur-kerja-pengembangan.md).
...
`apps/cms`'s own `runtime`/`jobs` images above are, since issue #187, additionally built and published to GHCR — since issue #225 (ADR-0023), from a trusted release host rather than CI, see "Published images" immediately below. `apps/storefront`'s image is deliberately not: it stays a manual `docker build` on the deploy host, for the reason given there.

### Published images (issue #187, ADR-0020; issue #225/ADR-0023 moved this off GitHub Actions)

`bun run release:images -- --publish --tag vX.Y.Z` (`tools/release/images.ts`, run by hand from a trusted release host — formerly `.github/workflows/images.yml`, triggered by a `v*` tag push or `workflow_dispatch`) builds `apps/cms/Dockerfile.production`'s `runtime` and `jobs` targets — the same, unmodified file the "Images" heading above describes — and publishes them to:
...
(`<owner>/<repo>` is this repository's own GitHub path, lower-cased, parsed from `git remote get-url origin` unless `--owner`/`--repo` are given — so a template-derived repository publishes to its own namespace with nothing to edit — for `ahliweb/awcms-one` that is `ghcr.io/ahliweb/awcms-one-cms` and `ghcr.io/ahliweb/awcms-one-cms-jobs`.) Each push carries three tags — the released semver (`vX.Y.Z` → `X.Y.Z`), the `X.Y` line, and the building commit's `sha` — plus an SBOM (exported to the run's evidence directory and, with `release:publish`'s `--evidence` flag, attached to the GitHub Release), signs each pushed digest with `cosign` (`COSIGN_KEY`), verifies the signature, and scans for vulnerabilities with trivy. See [ADR-0023](adr/0023-release-images-are-built-signed-and-published-from-a-trusted-release-host.md) for the replacement trust model and [`docs/rilis.md`](rilis.md) for the full command sequence and required environment. Without `--publish`, the same command only builds — never logs in, never pushes, never signs — which is the PR/verification mode this repository's `bun run ci`/`ci:pr` never needs to run itself, since it builds no image of its own.
...
**Verifying a pulled image's signature/attestation** — see [`docs/rilis.md`](rilis.md)'s "Verifying a published image": `cosign verify --key <published-cosign-public-key>` for anything published after ADR-0023, and the old `gh attestation verify oci://ghcr.io/<owner>/<repo>-cms:<tag> --owner <owner>` for an image published before it (that GitHub-issued attestation is still real and still verifiable — it is only how a NEW image gets its attestation that changed). A `PASS` either way names the exact commit/digest the image was built from — the same guarantee this document's own "Rollback/cutover" note already assumes ("every image is tagged by the commit/release it was built from"), now independently checkable rather than only asserted. The **SBOM** is exported to `tools/release/images.ts`'s own evidence directory and, with `release:publish --evidence`, attached to the GitHub Release as an asset.

**GHCR package visibility** — the first push to a new package (`ahliweb/awcms-one-cms`, `ahliweb/awcms-one-cms-jobs`) may create it as **private**, GHCR's own default for a package with no prior visibility setting. A private package needs its own pull credential (`docker login ghcr.io` with a token carrying `read:packages`) even for a deploy host this repository's own files name no secret for; the repository owner makes a package public from that package's own GitHub settings (its "Package settings" → "Change visibility") once, after which an anonymous `docker pull` works. Nothing in `tools/release/images.ts` sets visibility itself — GHCR ties it to manual owner action, not to anything a release-host run can request on its own behalf.

**Why `apps/storefront` is not published** — see [ADR-0020](adr/0020-publish-only-the-cms-images-to-ghcr-with-sbom-and-provenance.md) D2: its build bakes a tenant's live catalog/news content using the CMS owner token as a BuildKit secret, so publishing it would mean that production credential living somewhere CI/release automation reaches, and a published tag silently going stale the moment content changes with no corresponding new image. It stays built on the deploy host, exactly as the "Images" section above describes. `bun run release:images -- --storefront-smoke` (formerly `.github/workflows/images.yml`'s `storefront-smoke` job) only proves `apps/storefront/Dockerfile` still builds — matrixed over all three `SITE_PROFILE` values, against this repository's own stub CMS (`apps/storefront/scripts/stub-awcms.mjs`) — and never publishes the result.
...
1. **CI checks** — `bun run ci`'s twelve `local-ci/*` legs (`tools/ci/`, formerly `.github/workflows/ci.yml`/`e2e.yml`/`codeql.yml`/`template-init-smoke.yml`, all removed by issue #225/ADR-0021) lint, type-check, test, and run security analysis. They never touch a production credential and never mutate anything running.
2. **Artifact publication** — `bun run release:images -- --publish`/`release:publish` (`tools/release/`, ADR-0023, formerly `.github/workflows/images.yml`/`release.yml`, "Published images" above) builds and publishes signed, attested `apps/cms` images to GHCR and the matching GitHub Release, run by hand from a trusted release host. This is release engineering: producing an immutable, independently verifiable artifact. It still never deploys anything — nothing consumes a published image until a human or agent explicitly tells a production host to.
3. **Production deployment** — `tools/deploy/deploy-production.sh`, described immediately below. This is the ONLY step in this list that mutates a running production system, and it never runs inside GitHub Actions — there is no GitHub Actions execution anywhere in this repository any more (issue #225, ADR-0021): **no GitHub-hosted or self-hosted Actions runner is part of the production control plane, and no production host is ever registered as a GitHub self-hosted runner** — this repository is public, and GitHub's own guidance is explicit that a public repository's self-hosted runners are exposed to fork-PR code execution with the runner's own credentials and network reach.
