...
const EXCLUDED_PATHS = new Map([
  ...
  [
    "tools/out/seputarborneo-import-manifest.json",
    {
      reason:
        "issue #58 — the report tools/import-seputarborneo.ts writes on every run (counts and filenames only, never row content). git-ignored (tools/out/, see .gitignore) since it is regenerated by the script itself, never authored — so it never exists in a fresh checkout, only after the importer has run.",
      onlyIn: ["docs/deployment.md", "docs/deployment.id.md"]
    }
  ]
]);
