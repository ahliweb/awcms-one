...
<!-- i18n-source-hash: sha256:495bc25dc3f2f763c480db981fb4520af0de947155b980332fc15cdfd983cd50 -->
...
- **Belum ada:** yang secara eksplisit ditangguhkan D6 [ADR-0016](docs/adr/0016-customer-accounts-are-otp-verified-commerce-accounts-with-bearer-sessions.id.md) sebagai tindak lanjut pekerjaan akun — OTP WhatsApp/SMS, ubah e-mail/telepon pada akun yang sudah ada, verifikasi telepon, dan harga bertingkat (`priceLevel2/3/4`) yang diterapkan saat quote; POS dan pelaporan ([issue #33](https://github.com/ahliweb/awcms-one/issues/33), yang juga membawa integrasi ongkos kurir RajaOngkir dan payment gateway — keduanya ditangguhkan di belakang outbox per [ADR-0010](docs/adr/0010-manual-payment-and-alternative-courier-first-gateways-via-outbox.id.md)); gambar produk dan media slider yang di-resolve lewat unggahan sungguhan (sesi ber-R2 milik `media_library` sudah ada, tapi skrip seed tidak mengisinya — lihat [`docs/deployment.md`](docs/deployment.id.md)); deployment PostgreSQL produksi (`postgres:18.4` milik `compose.yaml` hanya kemudahan lokal/CI). Lihat [`docs/arsitektur.md`](docs/arsitektur.id.md), [`docs/cms.md`](docs/cms.id.md), dan [`docs/skema-basis-data.md`](docs/skema-basis-data.id.md) untuk daftar lengkap dan terkini apa yang masih ditangguhkan tiap area.

Setiap issue anak dari [issue #21](https://github.com/ahliweb/awcms-one/issues/21), dari [issue #46](https://github.com/ahliweb/awcms-one/issues/46), dan dari [issue #32](https://github.com/ahliweb/awcms-one/issues/32) sudah mendarat. Jangan menulis kode, gerbang, atau dokumentasi yang mengasumsikan salah satu dari butir "belum ada" itu sudah ada. Jalur yang dikutip dalam backtick yang tidak ada di repo ini akan tertangkap pemeriksaan jalur bertanda `bun run audit:dokumen`, kecuali terdaftar di `EXCLUDED_PATHS` gerbang itu, beserta alasannya. Lihat [`docs/README.md`](docs/README.id.md) untuk indeks dokumentasi lengkap.
...
`bun test` plus empat skrip `audit:*` berjalan tanpa syarat di setiap push, di job `check` — tidak satu pun darinya butuh build, jaringan, atau `apps/cms`. Job kedua, `check-cms`, menjalankan rangkaian gerbang penuh `apps/cms` sendiri plus rangkaian integrasi ber-gerbang-DB-nya terhadap layanan `postgres:18.4` yang hidup dan sekali-pakai (`.github/workflows/ci.yml`; lihat [`docs/alur-kerja-pengembangan.md`](docs/alur-kerja-pengembangan.id.md) untuk langkah persisnya). **Baik `check` maupun `check-cms` adalah status check wajib di `main`.**
