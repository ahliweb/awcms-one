...
- The three known local divergences that overlapped this delta — the
  `identity-access` `AccessAction`/high-risk-action unions, the module
  registry (`src/modules/index.ts`), and the two worker/subject-data grant
  matrices in `scripts/security-readiness.ts`/`scripts/subject-data-coverage-
  check.ts` — were resolved by keeping BOTH lineages (this platform's
  `commerce` additions and upstream's `omes_control` additions), not by
  choosing one side.
- Generated artifacts (`docs/awcms/api-reference.md`, `docs/awcms/repo-
  inventory.md`, `docs/awcms/module-composition-inventory.json`, `scripts/
  README(.id).md`, `docs/PROJECT_STATE(.id).md`'s inventory table) were
  regenerated from the merged registry rather than hand-merged, and the
  Indonesian mirrors touched by the merge were re-translated and re-stamped.
