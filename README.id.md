...
**Workflow GitHub Actions bukan jalur implementasi yang diterima di repositori ini (ADR-0021, issue #225).** CI, pemindaian keamanan, template smoke, dan tes end-to-end semuanya berjalan sebagai leg `local-ci/*` (`tools/ci/`) — di mesin kontributor sendiri, atau di server CI terpercaya milik AhliWeb — dilaporkan sebagai commit status oleh `bun run ci:pr -- <pr-number>`/`bun run ci:watch`. `bun test` plus empat skrip `audit:*` berjalan tanpa syarat sebagai bagian leg `local-ci/check-toko`, tidak butuh build, jaringan, atau `apps/cms`. Leg check adalah **matriks atas tiga profil build** — `local-ci/check-toko`, `local-ci/check-berita`, `local-ci/check-landing` — tiap leg men-type-check dan menjalankan profile-smoke-test `apps/storefront` di bawah `SITE_PROFILE` masing-masing; tes akar dan skrip `audit:*` berjalan sekali, di leg `toko`. `local-ci/check-cms` menjalankan rangkaian gerbang penuh `apps/cms` sendiri plus rangkaian integrasi ber-gerbang-DB-nya terhadap PostgreSQL hidup yang sekali-pakai — lihat [`docs/alur-kerja-pengembangan.md`](docs/alur-kerja-pengembangan.id.md). Empat leg `local-ci/template-*` menjalankan matriks `bun run template:init` atas ketiga profil ke dalam salinan sekali-pakai repo, plus satu leg `root` yang menjalankan suite tes derived-repository penuh sekali. Tiga leg `local-ci/e2e-*` menjalankan matriks Playwright atas ketiga profil yang sama (checkout, popup iklan, aksesibilitas axe-core, pemeriksaan responsif/overflow). **Kedua belas context `local-ci/*` semuanya status check wajib di `main`.**
...
| `local-ci/check-cms` | Rangkaian `bun run check` ~53 langkah `apps/cms` sendiri, lalu rangkaian `tests/integration/`-nya terhadap PostgreSQL hidup yang termigrasi sungguhan |

`local-ci/security` (analisis `security-extended` milik CLI CodeQL, gitleaks, dan `bun audit`) juga berjalan sebagai bagian `bun run ci`/`ci:pr` — lihat "Local CI: dua belas leg" di `docs/alur-kerja-pengembangan.md` untuk alasan CLI dijalankan langsung alih-alih lewat `codeql-action`. Build/publish image dan publikasi rilis tidak lagi dipicu CI sama sekali: `bun run release:images -- --publish` (image `runtime`/`jobs` milik `apps/cms` ke GHCR dengan SBOM dan provenance) dan `bun run release:publish` (GitHub Release dari bagian `CHANGELOG.md` sebuah versi yang di-tag) keduanya dijalankan dengan tangan dari server rilis terpercaya — lihat [`docs/rilis.md`](docs/rilis.id.md). Lihat [`docs/README.md`](docs/README.id.md) untuk indeks dokumentasi lengkap.
