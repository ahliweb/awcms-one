...
- **Not here yet:** what [ADR-0016](docs/adr/0016-customer-accounts-are-otp-verified-commerce-accounts-with-bearer-sessions.md) D6 explicitly deferred as follow-ups to the accounts work — WhatsApp/SMS OTP, e-mail/phone change on an existing account, phone verification, and tiered pricing (`priceLevel2/3/4`) applied at quote time; POS and reporting ([issue #33](https://github.com/ahliweb/awcms-one/issues/33), which also carries RajaOngkir courier-rate integration and a payment gateway — both deferred behind the outbox per [ADR-0010](docs/adr/0010-manual-payment-and-alternative-courier-first-gateways-via-outbox.md)); product images and sliders' media resolved through a real upload (`media_library`'s R2-backed session exists, but the seed script does not populate it — see [`docs/deployment.md`](docs/deployment.md)); a production PostgreSQL deployment (`compose.yaml`'s `postgres:18.4` is a local/CI convenience only). See [`docs/arsitektur.md`](docs/arsitektur.md), [`docs/cms.md`](docs/cms.md), and [`docs/skema-basis-data.md`](docs/skema-basis-data.md) for the full, current list of what each area still defers.

Every child issue of [issue #21](https://github.com/ahliweb/awcms-one/issues/21), of [issue #46](https://github.com/ahliweb/awcms-one/issues/46), and of [issue #32](https://github.com/ahliweb/awcms-one/issues/32) has now landed. Do not write code, gates, or documentation that assumes any of the "not here yet" items already exist. A path cited in backticks that does not exist in this repo is caught by `bun run audit:dokumen`'s named-path check unless it is listed in that gate's `EXCLUDED_PATHS`, with a reason. See [`docs/README.md`](docs/README.md) for the full documentation index.
