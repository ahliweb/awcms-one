...
- **Not here yet:** e-mail/phone change on an existing customer account and phone verification (the two [ADR-0016](docs/adr/0016-customer-accounts-are-otp-verified-commerce-accounts-with-bearer-sessions.md) D6 items increment 5 did not pick up); product images and sliders' media resolved through a real upload (`media_library`'s R2-backed session exists, but the seed script does not populate it — see [`docs/deployment.md`](docs/deployment.md)); a production PostgreSQL deployment (`compose.yaml`'s `postgres:18.4` is a local/CI convenience only); a Xendit payment-gateway adapter and courier tracking, both named as explicit follow-ups behind the same ports in [ADR-0017](docs/adr/0017-external-providers-are-commerce-owned-ports-with-env-credentials-and-token-addressed-webhooks.md). See [`docs/arsitektur.md`](docs/arsitektur.md), [`docs/cms.md`](docs/cms.md), and [`docs/skema-basis-data.md`](docs/skema-basis-data.md) for the full, current list of what each area still defers.
...
`bun test` plus four `audit:*` scripts run unconditionally on every push, in the `check` job — none of them needs a build, a network, or `apps/cms`. A second job, `check-cms`, runs `apps/cms`'s own full gate chain plus its DB-gated integration suite against a real, ephemeral `postgres:18.4` service (`.github/workflows/ci.yml`; see [`docs/alur-kerja-pengembangan.md`](docs/alur-kerja-pengembangan.md) for the exact steps). **Both `check` and `check-cms` are required status checks on `main`.**
